...
async function fetchPopularArtists() {
  ...
  for (const genre of genres) {
    const response = await fetch(
      `https://api.spotify.com/v1/search?q=genre:${genre}&type=artist&limit=${limit}`,
      {
        headers: {
...
function playPreviousTrack() {
  currentTrackIndex =
    (currentTrackIndex - 1 + currentTracks.length) % currentTracks.length;
  const prevTrack = currentTracks[currentTrackIndex];
  playMusic(
    prevTrack.preview_url,
    prevTrack.name,
    prevTrack.artists.map((artist) => artist.name).join(", "),
    currentTrackIndex
  );
}
